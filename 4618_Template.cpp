...
void lab3()
{
	...
	while (true)
	{
		...
		if (connected == false)
		{
			if (control.auto_init())
			{
				connected = true;
			}
			else
			{
				std::cout << "Board not found. Retrying in 1 second...\n";
				Sleep(1000);
				continue;
			}
		}
		...
		int choice = -1;
		while (choice != 0 && connected == true)
		{
			std::cout << "\n--- Lab 3 Test Menu ---" << std::endl;
			std::cout << "1. Analog Test (Joystick)" << std::endl;
			std::cout << "2. Digital Test (LED follow Button)" << std::endl;
			std::cout << "3. Button Test (Debounced Counter)" << std::endl;
			std::cout << "4. Servo Test (Min/Max Sweep)" << std::endl;
			std::cout << "0. Exit to Main Menu" << std::endl;
			std::cout << "Selection: ";
			std::cin >> choice;

			if (choice == 1) // ANALOG TEST
			{
				int raw_X = 0, raw_Y = 0;
				std::cout << "\nReading Joystick... (Press any key to exit)\n";

				while (!_kbhit())
				{
					// If joystick data acquired, continue
					bool found_X = control.get_data(ANALOG, JOY_X, raw_X);
					bool found_Y = control.get_data(ANALOG, JOY_Y, raw_Y);

					if (found_X == true && found_Y == true)
					{
						float percent_X = control.get_analog(JOY_X);
						float percent_Y = control.get_analog(JOY_Y);

						printf("ANALOG TEST: JOY_X = %4d (%5.1f %%) JOY_Y = %4d (%5.1f %%)\n",
							raw_X, percent_X, raw_Y, percent_Y);
					}
					else
					{
			...
			else if (choice == 2) // DIGITAL TEST
			{
				int buttonState = 0;
				std::cout << "DIGITAL TEST: Press any key to stop..." << std::endl;

				while (!_kbhit())
				{
					// Read the button state
					if (control.get_data(DIGITAL, S1, buttonState))
					{
						// Print to console for verification
						printf("DIGITAL TEST: S1 = %d\n", buttonState);

						// Set the LED to match the button
						control.set_data(DIGITAL, LED_GRN, !buttonState);
					}
					else
					{
						std::cout << "\nConnection Lost!" << std::endl;
					...
					// Small delay to keep the serial line from choking
					Sleep(10);
				}
				if (_kbhit())
				{
			...
			else if (choice == 3) // BUTTON TEST (DEBOUNCED)
			{
				int counter = 0;
				std::cout << "BUTTON TEST: Press Button S1. Press any key to stop.\n";

				while (!_kbhit())
				{
					int btnStatus = control.get_button(33); // Using pin 33 for S1

					if (btnStatus == BUTTON_PRESS_SUCCESS)
					{
						counter++;
						std::cout << "BUTTON COUNTER: " << counter << std::endl;
					}
					else if (btnStatus == BUTTON_PRESS_FAIL)
					{
						std::cout << "\nConnection Lost!" << std::endl;
						connected = false;
						break; // Exit loop to re-scan
					}
					// If it's BUTTON_PRESS_NONE, we do nothing and just loop again
			...
			else if (choice == 4) // DIGITAL TEST
			{
				...
				std::cout << "\nSERVO TEST. Press any key to stop." << std::endl;
				...
				while (!_kbhit())
				{
					...
					// Send to microcontroller (SERVO type = 2, Channel 0)
					if (control.set_data(SERVO, 0, position))
					{
						printf("SERVO TEST: POSITION %3d \n", position);
					}

					Sleep(10); // dont break servo
				}
				if (_kbhit())
				{
					_getch();
		...
		// If get_data fails, assume disconnection
		int val;
		if (control.get_data(DIGITAL, S1, val) == false)
		{
			std::cout << "\nWARNING: Connection Lost! Re-entering scan mode...\n";
			connected = false;
		}
	}
}
